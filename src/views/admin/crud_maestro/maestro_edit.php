        ...
        <section id="menu2" class="w-[100%] h-[100vh] bg-[#f5f6fa] ">
            ...
            <div class="p-5 h-[80%] flex flex-col gap-6 mt-[70px] ">
                <div class="flex justify-between">
                    <h1 class=" text-2xl font-medium text-gray-700">Editar Maestro</h1>
                ...
                <div class="bg-white shadow-sm shadow-gray-400 w-[100%] rounded-sm  flex flex-col justify-center gap-1">
                    ...
                    <div class="flex flex-col gap-4 p-3 pl-6">
                        <form class="flex flex-col gap-4 p-3 pl-6" method="post" action="/src/accions/crud_maestro/crud_edit_maestros.php">
                            <div>
                                <strong>
                                    <p>Correo Electronico</p>
                                </strong>
                                <div class="bg-gray-200 flex items-center border-gray-300 border-2 pr-3 rounded-md  hover:bg-slate-200  hover:shadow-custom hover:shadow-zinc-800">
                                    <input readonly class="bg-gray-200 px-3 py-[6px] w-[100%] rounded-l-md hover:bg-slate-200 focus:outline-0" type="text" name="email" value="<?php echo $resultado['email']; ?>">
                                </div>
                            </div>
                            <div>
                                <strong>
                            ...
                            <div>
                                ...
                                <div class="flex items-center border-gray-300 border-2 pr-3 rounded-md  hover:bg-slate-200  hover:shadow-custom hover:shadow-zinc-800">
                                    <select class="px-3 py-[6px] w-[100%] rounded-l-md hover:bg-slate-200 focus:outline-0" type="text" name="curso" value=""><?php
                                        $consulta1 = $mysqli->query("SELECT * from cursos");
                                        $resultado1 = $consulta1->fetch_assoc();
                                        while ($row = $consulta1->fetch_assoc()) {
                                            $selected = ($row['cursoID'] == $resultado1['cursoID']) ? 'selected' : '';
                                            echo "<option ' value='" . $row['cursoID'] . "' $selected>" . $row['cursoID'] . " - " . $row['nombreCurso'] . "</option>";
                                        }
                                        ?></select>
                                </div>
                            </div>
